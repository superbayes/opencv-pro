...
resampleContour_(const std::vector<cv::Point> &contour, int numSamples) {
  int n = static_cast<int>(contour.size());
  if (n < 3) {
  ...
  for (int i = 0; i < numSamples; ++i) {
    ...
    while (segIdx < n) {
      ...
      if (accumulated + segLen >= target) {
        ...
        result.emplace_back(static_cast<double>(contour[segIdx].x) + t * dx,
...
inline double zernikeRadial_(int n, int m, double rho) {
  ...
  for (int s = 0; s <= (n - mAbs) / 2; ++s) {
    ...
    double den = factorial_(s) * factorial_((n + mAbs) / 2 - s) *
...
/// 起始点规范化 — PCA 主轴对齐 + 极值点破平
/// 返回循环移位后的轮廓副本，起点统一为"主轴正方向投影最大点"
inline std::vector<cv::Point>
normalizeStartingPoint_(const std::vector<cv::Point> &contour) {
  int n = static_cast<int>(contour.size());
  if (n < 3)
    return contour;
  ...
  // 2. 构建 2×2 协方差矩阵 C = [[a, b], [b, c]]
  double a = 0.0, b = 0.0, c_ = 0.0;
  for (const auto &p : contour) {
  ...
  // 3. 解析求 2×2 协方差矩阵的最大特征向量方向
  //    λ_max = (trace + sqrt(trace² - 4·det)) / 2
  //    特征向量 ∝ (b, λ_max - a)，方向角 α = atan2(λ_max - a, b)
  double trace = a + c_;
  double det = a * c_ - b * b;
  double discriminant = trace * trace - 4.0 * det;
  if (discriminant < 1e-12) {
    // 退化情况 (正圆等): 行列式≈0 或两特征值相等
    // 此时 PCA 方向无意义，使用固定规则: 取 y 坐标最小的点
    int bestIdx = 0;
    double bestY = static_cast<double>(contour[0].y);
    for (int i = 1; i < n; ++i) {
      double yi = static_cast<double>(contour[i].y);
      if (yi < bestY - 1e-9) {
        bestY = yi;
        bestIdx = i;
      } else if (std::abs(yi - bestY) < 1e-9) {
        // 破平: y 相同时取 x 最小的
        if (static_cast<double>(contour[i].x) <
            static_cast<double>(contour[bestIdx].x)) {
          bestIdx = i;
        }
      }
    }
    std::vector<cv::Point> result = contour;
    std::rotate(result.begin(), result.begin() + bestIdx, result.end());
    return result;
  }

  double lambdaMax = (trace + std::sqrt(discriminant)) * 0.5;
  double vx, vy;
  // 当 b ≈ 0 时协方差矩阵退化为对角阵，特征向量直接为标准轴方向
  if (std::abs(b) < 1e-12) {
    // 对角矩阵: 最大特征值对应主轴方向
    if (a >= c_) {
      vx = 1.0;  vy = 0.0;   // x 轴为主轴
    } else {
      vx = 0.0;  vy = 1.0;   // y 轴为主轴
    }
  } else {
    vx = b;                         // 特征向量 x 分量
    vy = lambdaMax - a;             // 特征向量 y 分量
  ...
  double alpha = std::atan2(vy, vx);     // 主轴方向角

  // 4. 找沿主轴方向投影最大的点 (即"主轴正方向最远端")
  int bestIdx = 0;
  double bestProj = -1e30;
  double bestPerp = 0.0;                 // 垂直投影值 (用于破平)
  for (int i = 0; i < n; ++i) {
    double dx = static_cast<double>(contour[i].x) - cx;
    double dy = static_cast<double>(contour[i].y) - cy;
    double proj = dx * vx + dy * vy;     // 主轴方向投影
    double perp = -dx * vy + dy * vx;    // 垂直方向投影
    if (proj > bestProj + 1e-9) {
      bestProj = proj;
      bestPerp = perp;
      bestIdx = i;
    } else if (std::abs(proj - bestProj) < 1e-9) {
      // 破平: 投影相等时取垂直投影最小的 (即该方向上最接近主轴的唯一点)
      if (perp < bestPerp - 1e-9) {
        bestPerp = perp;
        bestIdx = i;
      }
    }
...
fourierDescriptors(const std::vector<cv::Point> &contour,
                   int numDescriptors = 10) {
  if (contour.size() < 3)
    return std::vector<double>(numDescriptors, 0.0);
  ...
  double dc = std::sqrt(
      dftResult.at<cv::Vec2d>(0, 0)[0] * dftResult.at<cv::Vec2d>(0, 0)[0] +
...
inline std::vector<double>
ellipticalFourierDescriptors(const std::vector<cv::Point> &contour,
                             int numHarmonics = 8) {
  if (contour.size() < 3)
    return std::vector<double>(numHarmonics * 4, 0.0);

  constexpr int RESAMPLE_N = 256;
  std::vector<cv::Point> aligned = normalizeStartingPoint_(contour);
  std::vector<cv::Point2d> pts = resampleContour_(aligned, RESAMPLE_N);
  int N = RESAMPLE_N;
...
inline std::vector<double>
centroidDistanceSignature(const std::vector<cv::Point> &contour,
                          int numSamples = 64) {
  if (contour.size() < 3)
    return std::vector<double>(numSamples, 0.0);

  std::vector<cv::Point> aligned = normalizeStartingPoint_(contour);
  std::vector<cv::Point2d> pts = resampleContour_(aligned, numSamples);
  cv::Point2d c = centroid_(pts);
...
inline std::vector<double>
complexCoordinateSignature(const std::vector<cv::Point> &contour,
                           int numSamples = 64) {
  if (contour.size() < 3)
    return std::vector<double>(numSamples * 2, 0.0);
...
inline std::vector<double>
tangentAngleSignature(const std::vector<cv::Point> &contour,
                      int numSamples = 64) {
  if (contour.size() < 3)
    return std::vector<double>(numSamples, 0.0);
...
inline std::vector<double> d2Distribution(const std::vector<cv::Point> &contour,
  ...
  // 归一化
  for (double &v : hist)
    v /= static_cast<double>(validPairs);
...
inline std::vector<double>
curvatureProfile(const std::vector<cv::Point> &contour, int numSamples = 64) {
  if (contour.size() < 5)
    return std::vector<double>(numSamples, 0.0);
...
inline double featureDistance(const std::vector<double> &v1,
  ...
  if (v1.size() != v2.size()) {
    throw std::runtime_error(
        "featureDistance: vectors must have the same size");
  }
  if (v1.empty())
    return 0.0;
...
inline double featureCosineSimilarity(const std::vector<double> &v1,
  ...
  if (v1.size() != v2.size()) {
    throw std::runtime_error(
        "featureCosineSimilarity: vectors must have the same size");
  }
  if (v1.empty())
    return 0.0;
