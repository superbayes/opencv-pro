...
- **所有特征函数自包含** — 内部自行完成等弧长重采样、**起始点规范化**与归一化，无需外界预处理
- **起始点不变性** — 序列化签名类函数（#2, #4, #5, #6, #9）内置 PCA 主轴对齐，同一形状不同起点输入产生完全相同输出
...
| # | 函数名 | 原理简介 | 维度 | 适用用途 | 直方图化 | 保留空间布局 | 抗变换能力 |
|---|--------|---------|:----:|---------|:--------:|:-------------:|-------------|
| 1 | `fourierDescriptors` | 轮廓→复信号→DFT→取前10阶谐波幅值，DC归一化 | **10** | **默认首选**，通用形状分类、旋转不变匹配 | 否 | 否 | ✅ 旋转、缩放、轻微变形 |
| 2 | `ellipticalFourierDescriptors` | x(t)/y(t) 分别傅里叶展开，8阶×4系数=a_n,b_n,c_n,d_n | **32** | 复杂形状重建与识别，对不规则轮廓鲁棒 | 否 | 否 | ⚠️ 缩放、轻微变形（旋转需PCA对齐） |
| 3 | `zernikeMoments` | 单位圆上 Zernike 正交矩分解，order=6，取幅值 | **25** | **旋转不变识别首选**，抗噪强，MPEG-7 标准 | 否 | 否 | ✅ 旋转、缩放、轻微变形 |
| 4 | `centroidDistanceSignature` | 64 点等弧长采样，每点到质心的距离，均值归一化 | **64** | 形状匹配、局部膨胀/收缩检测 | 否 | 部分 | ⚠️ 缩放、轻微变形（旋转需PCA对齐） |
| 5 | `complexCoordinateSignature` | 64 点复数坐标 z(t)=x+jy，质心原点，最大距离归一化 | **128** | 轮廓重建，保留完整位置信息 | 否 | 是 | ⚠️ 缩放、轻微变形（旋转需PCA对齐） |
| 6 | `tangentAngleSignature` | 64 点累积切向角序列，去线性趋势 | **64** | 角点检测、形状分段、拐点分析 | 否 | 是 | ✅ 旋转、缩放、轻微变形 |
| 7 | `d1Distribution` | 质心到轮廓点距离的32-bin概率直方图 | **32** | 形状检索、统计形状差异分析 | 是 | 否 | ✅ 旋转、缩放、轻微变形 |
| 8 | `d2Distribution` | 轮廓上随机点对距离的32-bin概率直方图 | **32** | 形状检索，对遮挡和噪声比 D1 更鲁棒 | 是 | 否 | ✅ 旋转、缩放、轻微变形 |
| 9 | `curvatureProfile` | 64 点离散曲率序列 κ≈Δθ/Δs，最大曲率归一化 | **64** | 局部形状特征、缺陷检测、凹凸分析 | 否 | 是 | ⚠️ 缩放（旋转需PCA对齐，变形敏感） |
...
**算法步骤：**
1. **PCA 主轴对齐起始点规范化** — 协方差矩阵求主轴方向，取主轴正方向投影最大点为统一起点
2. 等弧长重采样 → 256 点
3. 分别对 x(t)、y(t) 做傅里叶级数展开
4. 每阶谐波产生 4 个系数 (a_n, b_n, c_n, d_n)
5. 以第 1 阶椭圆大小做尺度归一化
...
1. **PCA 主轴对齐起始点规范化** — 协方差矩阵求主轴方向，取主轴正方向投影最大点为统一起点
2. 等弧长重采样 → `numSamples` 点
3. 逐点计算切向角 θ(t) = atan2(dy, dx)
4. 相位展开 (unwrap) 得到累积切向角 Θ(t)
5. 去除线性趋势（闭合轮廓的恒定旋转分量）
6. 归一化到 [-1, 1]
...
1. **PCA 主轴对齐起始点规范化** — 协方差矩阵求主轴方向，取主轴正方向投影最大点为统一起点
2. 等弧长重采样 → `numSamples` 点
3. 对每个采样点，用前后 k=3 邻点构造两向量
4. 计算离散曲率 κ ≈ Δθ / Δs（夹角 / 弧长）
5. 最大曲率归一化
...
以下 5 个序列化签名类函数内置了 **PCA 主轴对齐起始点规范化**：

| 函数 | 规范化效果 |
|------|---------|
| `ellipticalFourierDescriptors` | EFD 系数不随轮廓起点变化 |
| `centroidDistanceSignature` | 距离签名始终从主轴最远端开始 |
| `complexCoordinateSignature` | 复数坐标序列起点一致 |
| `tangentAngleSignature` | 累积切向角序列起点一致 |
| `curvatureProfile` | 曲率序列起点一致 |

**规范化算法**：计算轮廓点集的 2×2 协方差矩阵 → 解析求最大特征向量作为主轴方向 → 取沿主轴正方向投影最大的轮廓点作为统一起点 → 循环移位。退化情况（正圆形等）自动回退到固定规则（取 y 坐标最小点）。

以下 4 个函数**天然对起始点不敏感**，无需规范化：

| 函数 | 原因 |
|------|------|
| `fourierDescriptors` | DFT 取幅值波谱，循环移位仅影响相位 |
| `zernikeMoments` | 全局正交矩分解，非序列化统计 |
| `d1Distribution` | 直方图统计，丢失顺序信息 |
| `d2Distribution` | 直方图统计，丢失顺序信息 |
